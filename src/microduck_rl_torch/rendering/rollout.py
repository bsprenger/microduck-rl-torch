...
def _tensor_observation(value: object) -> torch.Tensor:
    if isinstance(value, dict):
        raise RuntimeError("Policy rendering requires a concatenated actor observation")
...
def render_policy_rollout(
    ...
    *,
    ...
    disable_mesh_mesh_contacts: bool | None = None,
    gif_fps: int = 25,
    gif_width: int = 720,
    gif_colors: int = 48,
    ...
    Floor contacts are always enabled for rendering. Detailed mesh-to-mesh
    contacts follow the task's authored policy unless explicitly overridden.
    """
    ...
    task_requires_mesh_mesh_skip = task_cfg.default_disable_mesh_mesh_contacts()
    if disable_mesh_mesh_contacts is False and task_requires_mesh_mesh_skip:
        raise ValueError(
            f"Task {task_cfg.task_name!r} requires mesh-mesh contacts to remain disabled"
        )
    if disable_mesh_mesh_contacts is None:
        disable_mesh_mesh_contacts = task_requires_mesh_mesh_skip
    ...
    primary_name = task_cfg.primary_entity or task_cfg.scene.primary_entity
    ...
    observation = _tensor_observation(environment.reset())
    if progress:
        print(
    ...
    succeeded = False
    terminated = False
    root_view = bundle.entity(bundle.primary_entity_cfg.name)
    ...
    try:
        for _step in range(steps):
            ...
            if result.terminated:
                terminated = True
                break
        if seconds is not None and completed_steps != steps:
            raise RuntimeError(
                f"Duration-based render terminated early at step {completed_steps}/{steps}"
            )
        succeeded = True
    finally:
        try:
    ...
    return {
        ...
        "actual_seconds": completed_steps * control_timestep,
