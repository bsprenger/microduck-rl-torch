...
class NativeRenderer:
    ...
    def __init__(self, bundle: ModelBundle, *, width: int, height: int, camera: CameraConfig):
        ...
        if camera.name is not None:
            _camera_id, camera_name = resolve_named_camera(
                self._model,
                camera.name,
            ...
            if camera.track_body is not None:
                ...
                view.trackbodyid = _required_body_id(
                    self._model,
                    camera.track_body,
                    entity_name=self.bundle.primary_entity_cfg.name,
                )
                view.fixedcamid = -1
            else:
    ...
    def render(self, env: Any) -> np.ndarray:
        ...
        # but copying them when present keeps the bridge useful for additional
        ...
        if (
            ...
            root_name = self.bundle.primary_entity_cfg.name
