...
def bad_orientation(env: Any) -> torch.Tensor:
    ...
    runtime = env.task_cfg.runtime
    limit = torch.cos(
        torch.deg2rad(
            torch.tensor(
                runtime.bad_orientation_degrees,
...
def timeout(env: Any) -> torch.Tensor:
    ...
    episode_length_steps = env.task_cfg.runtime.episode_length_steps
    if num_envs > 1:
        step_counts = getattr(
            env,
            "step_counts",
        ...
        return step_counts >= episode_length_steps
    return torch.as_tensor(env.step_count >= episode_length_steps)
