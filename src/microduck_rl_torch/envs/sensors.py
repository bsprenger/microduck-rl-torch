...
Task configuration names sensors, ``SensorManager`` resolves all model
addresses once, and task terms read named values without touching MuJoCo
indices.
"""
...
def contact_valid(data: Any) -> torch.Tensor:
    """Return valid contact slots in the backend's fixed-size contact buffer."""
    ...
    return (
        valid_slots
        & (data.contact.efc_address >= 0)
        & (data.contact.dist < data.contact.includemargin)
    )


def contact_mask(data: Any, geom_groups: Any) -> torch.Tensor:
    """Return one contact flag per semantic geometry group."""
    ...
    for group in geom_groups:
        geom_ids = torch.as_tensor(group, device=geom1.device)
        result.append(
            valid
            & (torch.isin(geom1, geom_ids) | torch.isin(geom2, geom_ids))
            & (data.contact.dist < data.contact.includemargin)
        )
    return (
...
class SensorManager:
    ...
    Supported built-ins cover named MuJoCo sensors, body pose/velocity, site
    position/velocity,
    ...
    def __init__(self, config: dict[str, SensorCfg], bundle: ModelBundle) -> None:
        ...
        self._ticks: dict[str, int | torch.Tensor] = {}
    ...
    def _resolve(self, cfg: SensorCfg) -> _ResolvedSensor:
        ...
        if kind == "mujoco":
            ...
            if cfg.reduce not in {"none", "mindist", "maxforce", "netforce"}:
            ...
            if (
                ...
                raise ValueError(
                    f"Contact sensor {cfg.name!r} with global_frame=True needs "
                    "normal and tangent fields"
                )
    ...
    def _entity_cfg(self, name: str | None) -> EntityCfg:
        ...
        if cfg is None and name == self.bundle.primary_entity_cfg.name:
            cfg = self.bundle.primary_entity_cfg
        if cfg is None:
    ...
    def _read_contact_from_sensors(self, env: Any, resolved: _ResolvedSensor) -> ContactData | None:
        ...
        if not resolved.contact_slices:
            return None
        cfg = resolved.cfg
        # The backend exposes the contact count but not the other CONTACT
        # dataspec fields. Use the native contact graph for those fields so
        # typed observations remain physical values instead of placeholders.
        if any(field_name != "found" for field_name in cfg.fields):
            return None
        ...
        for field_name, chunks in values.items():
            ...
            if cfg.reduce != "none":
                # Reduced contact sensors have one value per primary, not a
                # visible singleton slot axis.  Keeping that axis here makes
                # the generated-sensor path disagree with the native path
                # and breaks batched critic observations.
                stacked = stacked.squeeze(1)
    ...
    def _read_raycast(self, env: Any, resolved: _ResolvedSensor) -> RayCastData:
        ...
        if cfg.kind == "terrain_height":
            # Match mjlab's terrain-height contract.  A ray that hits the
            # underside of geometry means the frame is inside terrain and
            # therefore has zero clearance.  A partial miss uses the maximum
            # range, while an all-miss frame uses the frame height clamped to
            # the configured range so foot-height rewards remain meaningful
            # when the robot is above the finite terrain patch.
            frame_count = len(resolved.ray_frames)
            heights = (
                frame_position_tensor[:, 2].repeat_interleave(ray_count) - hit_tensor[:, 2]
            ).reshape(frame_count, ray_count)
            distance_grid = distance_tensor.reshape(frame_count, ray_count)
            normals_z = normal_tensor[:, 2].reshape(frame_count, ray_count)
            backface = (distance_grid >= 0) & (normals_z < 0)
            heights = torch.where(backface, torch.zeros_like(heights), heights)
            miss = distance_grid < 0
            all_miss = miss.all(dim=-1, keepdim=True).expand_as(miss)
            fallback = frame_position_tensor[:, 2].unsqueeze(-1).clamp(0, cfg.max_distance)
            fallback = fallback.expand_as(heights)
            miss_value = torch.where(
                all_miss,
                fallback,
    ...
    def _contact_mask(self, data: Any, resolved: _ResolvedSensor) -> torch.Tensor:
        ...
        valid_slots = slots < count[..., None] if count.ndim else slots < count
        valid = valid_slots & (data.contact.efc_address >= 0)
        valid &= data.contact.dist < data.contact.includemargin
    ...
    def _read_contact_native(self, env: Any, resolved: _ResolvedSensor) -> ContactData:
        ...
        for primary_set in primary_sets:
            ...
            for contact_id in range(int(native_data.ncon)):
                ...
                if geom1 in primary_set:
                    primary_is_geom1 = True
                    other = geom2
                elif geom2 in primary_set:
                    primary_is_geom1 = False
                    other = geom1
                else:
                    continue
                ...
                force_local = force6[:3].copy()
                torque_local = force6[3:].copy()
                if not primary_is_geom1 and resolved.cfg.reduce == "netforce":
                    # Apply the primary/secondary direction to the complete
                    # wrench before rotating it to world axes.
                    force_local *= -1.0
                    torque_local *= -1.0
                elif not primary_is_geom1:
                    # Match mujoco_warp's ContactSensor kernel. It keeps the
                    # first two local wrench components in solver order and
                    # applies the primary/secondary direction only to the
                    # third component. Normal and tangent are directed from
                    # the selected primary toward the secondary.
                    force_local[2] *= -1.0
                    torque_local[2] *= -1.0
                if resolved.cfg.reduce == "netforce":
                    # MuJoCo's contact frame stores the normal in row 0 and
                    # the two tangents in rows 1/2.  ``mj_contactForce`` uses
                    # that same local component order, so rows.T maps the
                    # force back to world axes.  Do not rotate through a
                    # tangent/normal permutation: that produces a plausible
                    # three-vector while turning a vertical foot load into a
                    # lateral force.
                    force = frame.T @ force_local
                    torque = frame.T @ torque_local
                elif resolved.cfg.global_frame:
                    # Match mjlab's public global-frame transform. The
                    # non-netforce dataspec is expressed in the basis
                    # (tangent, tangent2, normal), while the native contact
                    # force above is still in MuJoCo's local component order.
                    normal = frame[0].copy() if primary_is_geom1 else -frame[0].copy()
                    tangent = frame[1].copy() if primary_is_geom1 else -frame[1].copy()
                    tangent2 = np.cross(normal, tangent)
                    basis = np.stack((tangent, tangent2, normal), axis=-1)
                    force = basis @ force_local
                    torque = basis @ torque_local
                else:
                    force = force_local
                    torque = torque_local
                records.append(
        ...
        def tensor(value: Any, *, size: int | None = None) -> torch.Tensor:
            # Materialize homogeneous native records as one ndarray before
            # handing them to Torch; constructing from a list of ndarrays is
            # both noisy and needlessly quadratic for rough-terrain contacts.
            result = torch.as_tensor(
                np.asarray(value), dtype=self.bundle.dtype, device=self.bundle.device
            )
            if size is not None:
        ...
        contact_counts = [len(records) for records in contacts_by_primary]
        netforce_values: list[dict[str, Any]] = []
        if resolved.cfg.reduce == "netforce":
            for records in contacts_by_primary:
                if not records:
                    netforce_values.append(
                        {
                            "force": np.zeros(3),
                            "torque": np.zeros(3),
                            "dist": 0.0,
                    ...
                    continue
                weights = np.asarray([np.linalg.norm(record["force"]) for record in records])
                ...
                force = np.sum([record["force"] for record in records], axis=0)
                torque = np.sum(
                    [
                        record["torque"] + np.cross(record["pos"], record["force"])
        ...
        def values_for(field_name: str, primary_index: int) -> Any:
            ...
            if field_name == "found":
                # ``found`` is a contact count, not a boolean. Reduce it to a
                # per-foot contact flag only at the final observation boundary.
                return float(contact_counts[primary_index])
            if resolved.cfg.reduce == "netforce":
                return netforce_values[primary_index][field_name]
            if not records:
                dim = self._contact_field_dim(field_name)
    ...
    def _read_resolved(self, env: Any, resolved: _ResolvedSensor) -> torch.Tensor:
        ...
        if kind == "mujoco":
            ...
            if entity is None:
                raise RuntimeError(f"Sensor {cfg.name!r} requires an entity")
            value = torch.cat(
                [
                    torch.cat(
                    ...
                    for body_id in (resolved.ids or (entity.root_body_id,))
                ],
                dim=-1,
            )
            ...
            if entity is None:
                raise RuntimeError(f"Sensor {cfg.name!r} requires an entity")
            body_ids = list(resolved.ids or (entity.root_body_id,))
            ...
            value = site_linear_velocity(data, self.bundle, site_ids)
    ...
    def _stack_ray_data(rows: list[RayCastData]) -> RayCastData:
        return RayCastData(
            ...
            heights=(
                torch.stack([cast(torch.Tensor, row.heights) for row in rows])
                if all(row.heights is not None for row in rows)
                else None
            ),
        )
    ...
    def reset(self, env: Any, env_ids: torch.Tensor | slice | None = None) -> None:
        ...
        for resolved in self._resolved.values():
            ...
            if num_envs > 1:
                tick_value = self._ticks.get(resolved.cfg.name)
                ticks = (
                    tick_value
                    if isinstance(tick_value, torch.Tensor)
                    else torch.full(
                        (num_envs,),
                        int(tick_value or 0),
                        dtype=torch.long,
    ...
    def update(
        ...
        *,
        ...
        """Refresh sensors, optionally only for selected vector rows.

        A partial environment reset must not advance contact timers or
        history buffers for sibling rows.  The backend data read is still
        vectorized, but cache/tick/timing writes are masked to ``env_ids``.
        """

        dt = self.bundle.timestep if dt is None else float(dt)
        num_envs = getattr(env, "num_envs", 1)
        partial = num_envs > 1 and env_ids is not None
        if partial:
            selected_ids = self._ids(env_ids, num_envs).to(self.bundle.device)
            selected = torch.zeros(num_envs, dtype=torch.bool, device=self.bundle.device)
            selected[selected_ids] = True
        else:
            selected = None
        if advance:
            # Update stateful sensors before their data is consumed so
            # raycasts and custom temporal sensors observe the current substep.
            if getattr(env, "num_envs", 1) > 1:
                for resolved in self._resolved.values():
        ...
        for name, resolved in self._resolved.items():
            ...
            if num_envs > 1:
                ...
                due = (
                    torch.ones(num_envs, dtype=torch.bool, device=value.device)
                    if not advance
                    else (tick % period == 0) | (name not in self._values)
                )
                if selected is not None and name in self._values:
                    due &= selected
                if name in self._values:
                    ...
                    value = current
                if advance:
                    self._ticks[name] = tick + due.to(dtype=tick.dtype)
            else:
                if advance and tick % period != 0 and name in self._values:
                    continue
                if advance:
            ...
            history_value = value.clone()
            if selected is not None and history:
                history_value[~selected] = history[0][~selected]
            history.insert(0, history_value)
            ...
            if resolved.cfg.kind == "contact" and resolved.cfg.track_air_time:
                ...
                if contact is not None and contact.found is not None:
                    found_values = contact.found
                    if resolved.cfg.reduce == "none":
                        contact_found = (found_values > 0).any(dim=-1)
                    else:
                        contact_found = found_values > 0
                ...
                if active is not None:
                    active = active.reshape((-1,) + (1,) * (contact_found.ndim - 1))
                next_last_air = torch.where(
                    was_air & contact_found,
                    timing["air"] + dt,
                    timing["last_air"],
                )
                next_last_contact = torch.where(
                    was_contact & ~contact_found,
                    timing["contact"] + dt,
                    timing["last_contact"],
                )
                next_air = torch.where(
                    contact_found,
                ...
                if active is not None:
                    timing["last_air"] = torch.where(active, next_last_air, timing["last_air"])
                    timing["last_contact"] = torch.where(
                        active, next_last_contact, timing["last_contact"]
                    )
                    timing["air"] = torch.where(active, next_air, timing["air"])
                    timing["contact"] = torch.where(active, next_contact, timing["contact"])
                else:
                    timing["last_air"] = next_last_air
                    timing["last_contact"] = next_last_contact
                    timing["air"] = next_air
                    timing["contact"] = next_contact
    ...
    def sense(self, env: Any, env_ids: torch.Tensor | slice | None = None) -> None:
        """Refresh current sensor values without advancing temporal state.

        MuJoCo's post-step ``sense`` pass recomputes derived sensor values but
        does not run another sensor update. Keeping that distinction prevents
        stateful custom readers, contact timers, and history buffers from
        advancing twice per policy step.
        """

        self.update(env, env_ids, dt=0.0, advance=False)
        for name, resolved in self._resolved.items():
            if resolved.cfg.kind != "contact" or not resolved.cfg.track_air_time:
                continue
            if name in self._air_time and env_ids is None:
                continue
            contact = resolved.contact_data
            if contact is not None and contact.found is not None:
                found_values = contact.found
                if resolved.cfg.reduce == "none":
                    ...
                    contact_found = found_values > 0
                if getattr(env, "num_envs", 1) == 1 and contact_found.ndim > 1:
                    contact_found = contact_found.squeeze(0)
            else:
                mask = self._contact_mask(env.data, resolved)
                if getattr(env, "num_envs", 1) > 1:
                    ...
                    contact_found[:] = mask.any(dim=-1, keepdim=True)
                else:
                    contact_found = torch.full(
                        (resolved.contact_primary_count,),
                        bool(mask.any()),
                        dtype=torch.bool,
                        device=self.bundle.device,
                    )
            zero = torch.zeros_like(contact_found, dtype=self.bundle.dtype)
            timing = {
                "air": zero.clone(),
                "contact": zero.clone(),
                "last_air": zero.clone(),
                "last_contact": zero.clone(),
            }
            if env_ids is None or getattr(env, "num_envs", 1) == 1:
                self._air_time[name] = zero.clone()
                self._contact_timing[name] = timing
            else:
                ids = self._ids(env_ids, env.num_envs).to(self.bundle.device)
                if name not in self._air_time:
                    self._air_time[name] = torch.zeros_like(zero)
                    self._contact_timing[name] = {
                        key: torch.zeros_like(value) for key, value in timing.items()
                    }
                self._air_time[name][ids] = 0
                for key in timing:
                    self._contact_timing[name][key][ids] = 0
            if resolved.contact_data is not None:
                resolved.contact_data.current_air_time = self._air_time[name]
                resolved.contact_data.last_air_time = self._contact_timing[name]["last_air"]
                resolved.contact_data.current_contact_time = self._contact_timing[name]["contact"]
                resolved.contact_data.last_contact_time = self._contact_timing[name]["last_contact"]
