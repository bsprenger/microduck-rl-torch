"""Explicit configuration for the Microduck velocity task."""
...
class CommandTermCfg:
    ...
    # A term may be a function or a callable stateful object/class.
    func: Callable[..., Any] | None = None
    # Plain function terms must declare their width. Stateful
    # command classes may omit it; the manager derives it from ``command``
...
class CommandConfig(MutableMapping[str, CommandTermCfg]):
    """Generic ordered command-term collection.

    Command sampling ranges belong to a task's command generator, not to the
    environment-wide command manager.  Velocity tasks use
    :class:`VelocityCommandConfig`; posture, manipulation, and testbench
    tasks can use this small collection without carrying unrelated defaults.
    """
...
class VelocityCommandConfig:
    """Sampling defaults for the Microduck velocity command."""
...
class DomainRandomizationConfig:
    """Reset/interval randomization ranges for the velocity task."""
...
class MicroDuckVelocityConfig:
    ...
    command: VelocityCommandConfig = field(default_factory=VelocityCommandConfig)
...
def sample_twist(
    config: VelocityCommandConfig | None,
    *,
    generator: torch.Generator | Sequence[torch.Generator] | None,
    device: torch.device,
    dtype: torch.dtype,
