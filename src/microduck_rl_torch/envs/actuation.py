"""Microduck BAM M6 actuator equations.

The task uses the ``bam`` M6 model rather than MuJoCo's XML position actuator.
Keeping the equations here makes the policy-facing environment independent of
the optional reference training stack.
"""
...
class ActuatorDelayCfg:
    """Physics-step command latency for one actuator group.

    Delay is attached to the entity/actuator boundary, not to the policy
    action manager. The delayed command is the value consumed by the actuator
    control law and is sampled independently for each environment instance.
    """
    ...
    def __post_init__(self) -> None:
        if self.delay_min_lag < 0 or self.delay_max_lag < self.delay_min_lag:
            raise ValueError("Actuator delay must satisfy 0 <= min_lag <= max_lag")
        if not 0.0 <= self.delay_hold_prob <= 1.0:
            raise ValueError("Actuator delay hold probability must be in [0, 1]")
        if self.delay_update_period < 0:
            raise ValueError("Actuator delay update period must be non-negative")


@dataclass(frozen=True)
class BamM6Parameters:
    """Calibrated BAM M6 XL330 parameters."""
...
def friction_budget(
    ...
    *,
    ...
    # The DR term models uncertainty in the velocity-independent friction
    # budget only. Viscous friction is independent, so the friction-budget
    # scale must never change passive joint damping.
    return friction * friction_scale, params.friction_viscous
