...
def _tensor_observation(value: object) -> torch.Tensor:
    if isinstance(value, dict):
        raise RuntimeError("Golden trajectory comparison requires a concatenated observation")
...
def _native_termination(env: NativeMicroDuckEnv) -> tuple[bool, bool]:
    ...
    quaternion = env.data.xquat[env.root_body_id]
...
def rollout_torch(
    ...
    *,
    ...
    # The independent native fixture records the raw weighted reward.  The
    # Disable task-level reward scaling for this physics comparison.
    task_cfg.reward_scale_by_dt = False
    if expected_contacts is False:
        # The native reference's contact-disabled mode suppresses all contact
        # dependent foot shaping.  Preserve that fixture contract without
        # changing the normal task reward graph (where clearance is valid even
        # when a foot is currently airborne).
        for name in ("air_time", "foot_clearance", "foot_swing_height"):
            task_cfg.rewards[name].enabled = False
    # The native environment exposes the raw IMU channels.  Keep this physics
    # golden trace independent of the task's sampled observation delay; the
    # manager still supports that delay for normal task/inference rollouts.
    for name in ("base_ang_vel", "projected_gravity"):
        term = task_cfg.observations.groups["actor"].terms[name]
    ...
    observations = [_tensor_observation(environment.reset()).detach().cpu().numpy()]
    ...
    for action in trajectory.actions:
        ...
        observations.append(_tensor_observation(result.observation).detach().cpu().numpy())
        ...
        terminated.append(_bool_value(result.terminated))
        truncated.append(_bool_value(result.truncated))
    return {
        "observations": np.asarray(observations, dtype=np.float32),
        "qpos": np.asarray(qpos),
        "qvel": np.asarray(qvel),
