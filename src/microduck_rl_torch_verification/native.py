...
class NativeMicroDuckEnv:
    ...
    def __init__(
        ...
        *,
        ...
        if bundle is not None:
            ...
            self.xml_path = (xml_path or MICRODUCK_WALK_ROBOT_CFG.keyframe_source).resolve()
        ...
        self.actuator_delay = bundle.actuator_delay if bundle is not None else ActuatorDelayCfg()
        ...
        self._delay_rng = np.random.default_rng(0)
        self._actuator_delay_buffer: list[np.ndarray] = []
        self._actuator_delay_lag = 0
        self._actuator_delay_step = 0
        self._actuator_delay_phase = 0
        self.imu_ang_vel = self._sensor_slice("imu_ang_vel")
        self.imu_accel = self._sensor_slice("imu_accel")
        self.root_body_id = mujoco_api.mj_name2id(
    ...
    def _compute_reward(
        ...
        trunk_rotation = self.data.xmat[self.root_body_id].reshape(3, 3)
        gravity = trunk_rotation.T @ np.array([0.0, 0.0, -1.0])
        upright = float(np.exp(-np.square(gravity[:2]).sum() / 0.05))
        body_velocity = self.data.cvel[self.root_body_id, 3:6]
        ...
        angular_momentum = float(np.square(self.data.subtree_angmom[self.root_body_id]).sum())
    ...
    def reset(self) -> np.ndarray:
        ...
        self._step_count = 0
        self._actuator_delay_buffer.clear()
        self._actuator_delay_lag = 0
        self._actuator_delay_step = 0
        self._actuator_delay_phase = (
            int(self._delay_rng.integers(self.actuator_delay.delay_update_period))
            if self.actuator_delay.delay_update_period > 0
            and self.actuator_delay.delay_per_env_phase
            else 0
        )
        return self.observation()

    def _apply_actuator_delay(self, target: np.ndarray) -> np.ndarray:
        """Apply the entity actuator delay at each native physics substep."""

        config = self.actuator_delay
        if config.delay_max_lag <= 0:
            return target
        if config.delay_update_period > 0:
            should_update = (
                self._actuator_delay_step + self._actuator_delay_phase
            ) % config.delay_update_period == 0
        else:
            should_update = True
        if should_update and self._delay_rng.random() >= config.delay_hold_prob:
            self._actuator_delay_lag = int(
                self._delay_rng.integers(config.delay_min_lag, config.delay_max_lag + 1)
            )
        self._actuator_delay_buffer.append(target.copy())
        if len(self._actuator_delay_buffer) > config.delay_max_lag + 1:
            self._actuator_delay_buffer.pop(0)
        self._actuator_delay_step += 1
        available_lag = min(self._actuator_delay_lag, len(self._actuator_delay_buffer) - 1)
        return self._actuator_delay_buffer[-1 - available_lag]

    def observation(self) -> np.ndarray:
        gravity = _quat_rotate_inverse(
            self.data.xquat[self.root_body_id], np.array([0.0, 0.0, -1.0])
        )
    ...
    def step(self, action: np.ndarray) -> np.ndarray:
        ...
        for _ in range(self.decimation):
            ...
            if self.actuator_mode == "bam":
                self.data.ctrl[:] = self._bam_control(delayed_target)
            else:
