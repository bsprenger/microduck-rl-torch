...
def test_missing_policy_is_not_silently_accepted(tmp_path):
    (tmp_path / "manifest.json").write_text(
        json.dumps({"policies": [{"file": "alpha_walking.onnx"}]})
    )
...
def test_manifest_policy_filenames_support_official_list_shape():
    ...
    assert _policy_entry(manifest, "ball_kick_left.onnx")["file"] == "ball_kick_left.onnx"


def test_policy_filename_stem_resolves_to_declared_file():
    manifest = {"policies": [{"file": "alpha_sitstand.onnx"}]}

    assert resolve_policy_filename(manifest, "alpha_sitstand") == "alpha_sitstand.onnx"
...
def test_policy_set_downloads_each_policy_without_shared_metadata_file(monkeypatch, tmp_path):
    ...
    manifest = {
        "robot": "microduck",
        "policies": [
            {"file": "alpha_first.onnx"},
            {"file": "alpha_second.onnx"},
        ],
    }
    ...
    assert set(artifacts) == {"alpha_first", "alpha_second"}
    assert artifacts["alpha_first"].policy_name == "alpha_first.onnx"
    assert artifacts["alpha_second"].policy_name == "alpha_second.onnx"
