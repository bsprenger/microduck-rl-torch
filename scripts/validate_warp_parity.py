"""Run a paired 500-step Torch versus microduck_rl MuJoCo-Warp parity check."""
...
def _tensor_observation(value: Any) -> torch.Tensor:
    if isinstance(value, dict):
        raise RuntimeError("Warp parity requires a concatenated actor observation")
    return value


def _bool_value(value: bool | torch.Tensor) -> bool:
...
def _local_trace(
    *,
    ...
    # The microduck_rl walk task's FULL_COLLISION expression matches only the two
    ...
    with torch.inference_mode():
        ...
        for step in range(steps):
            observation = _tensor_observation(observation)
            ...
            terminated_value = _bool_value(result.terminated)
            truncated_value = _bool_value(result.truncated)
            ...
            observation = _tensor_observation(result.observation)
...
def _run_microduck_rl(
    *,
    microduck_rl_python: Path,
    ...
    runner = Path(__file__).with_name("run_microduck_rl_warp_rollout.py").resolve()
    source_root = microduck_rl_root.resolve() / "src"
    environment = os.environ.copy()
    environment["PYTHONPATH"] = os.pathsep.join(
        path for path in (str(source_root), environment.get("PYTHONPATH", "")) if path
    )
    command = [
        str(microduck_rl_python.absolute()),
        str(runner),
        "--policy-path",
    ...
    if completed.returncode != 0:
        detail = output[-4000:] if output else "microduck_rl runner exited without diagnostics"
        raise RuntimeError(
            "microduck_rl MuJoCo-Warp rollout failed with "
...
def run_check(args: argparse.Namespace) -> int:
    ...
    metadata: dict[str, Any] = {
        ...
        "microduck_rl_root": str(args.microduck_rl_root.resolve()),
        "microduck_rl_python": str(args.microduck_rl_python.absolute()),
        ...
        "microduck_rl_device": args.microduck_rl_device,
        "comparison": (
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="microduck-warp-parity-") as temporary:
            microduck_rl_path = Path(temporary) / "warp-trace.npz"
            microduck_rl_trace, microduck_rl_output = _run_microduck_rl(
                microduck_rl_python=args.microduck_rl_python,
                microduck_rl_root=args.microduck_rl_root,
                policy_path=policy_path,
                steps=args.steps,
                seed=args.seed,
                vx=args.vx,
                vy=args.vy,
                vtheta=args.vtheta,
                device=args.microduck_rl_device,
                output_path=microduck_rl_path,
            )
        metadata.update(
            {
                "local_trace": local_trace.metadata,
                "microduck_rl_trace": microduck_rl_trace.metadata,
                "microduck_rl_runner_output": microduck_rl_output,
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--microduck-rl-root", type=Path, required=True)
    parser.add_argument("--microduck-rl-python", type=Path, required=True)
    parser.add_argument(
    ...
    parser.add_argument("--microduck-rl-device", default="cpu")
