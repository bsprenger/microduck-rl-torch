"""Run the microduck_rl mjlab/MuJoCo-Warp task and save a parity trace.

This file intentionally imports only microduck_rl packages plus NumPy. The
parent verification command launches it with the microduck_rl project's
interpreter and source tree, which keeps the reference rollout independent
from the local Torch implementation.
"""
...
def _actor_observation(observations: Any) -> np.ndarray:
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "microduck_rl observation manager returned no actor/policy group"
            ) from error
    result = _one_env_array(value).reshape(-1)
    if result.shape != (61,):
        raise RuntimeError(
            f"Expected microduck_rl actor observation shape (61,), got {result.shape}"
        )
...
def _set_command(term: Any, value: np.ndarray, *, name: str, device: Any) -> None:
    ...
    if command is None:
        raise RuntimeError(f"Could not locate live tensor for microduck_rl command {name!r}")
    command[0].copy_(target)
...
def _make_deterministic_config(steps: int) -> Any:
    ...
    # Disable stochastic events and curriculum so both backend rollouts use the
    # same deterministic reset and stepping contract.
    ...
    # The comparison does not include curriculum updates or randomization.
    ...
    # The parity configuration uses zero actuator delay and retains the
    # one-tick joint-velocity observation delay.
    ...
    try:
        ...
        for _step in range(steps):
            ...
            if action.shape != (14,) or not np.isfinite(action).all():
                raise RuntimeError(
                    f"microduck_rl policy returned invalid action shape/value: {action.shape}"
                )
        ...
        metadata = {
            "schema_version": 1,
            "backend": "microduck_rl-mjlab-mujoco-warp",
            "task": "Mjlab-Velocity-Flat-MicroDuck",
            "microduck_rl_root": str(microduck_rl_root.resolve()),
            "microduck_rl_revision": _git_revision(microduck_rl_root),
