...
Examples::
    ...
    uv run --group benchmark python scripts/benchmark_physics.py \
        --backend both --devices cpu,mps --microduck-rl-root ../microduck_rl \
...
def _unsupported(backend: str, device: str, reason: str) -> dict[str, Any]:
    labels = {
        ...
        "microduck_rl_mujoco_warp": "microduck_rl MuJoCo-Warp",
    }
    return {
        "backend": backend,
...
def _make_microduck_rl_config(
    *,
    ...
    if not mesh_mesh_contacts:
        ...
        # anonymous mesh contacts so the benchmark retains plane-foot contacts while removing
...
def _run_microduck_rl(
    *,
    ...
    if not microduck_rl_root.is_dir():
        return _failed(
            "microduck_rl_mujoco_warp",
            device,
            FileNotFoundError(microduck_rl_root),
        )
    ...
    config = _make_microduck_rl_config(
    ...
    try:
        ...
        return {
            "backend": "microduck_rl_mujoco_warp",
            "backend_label": "microduck_rl MuJoCo-Warp",
            "device": device,
            "status": "ok",
...
def _run_one(
    *, backend: str, device: str, args: argparse.Namespace, controls_np: np.ndarray
    ...
    try:
        ...
        return _failed(
            "microduck_rl_torch" if backend == "local" else "microduck_rl_mujoco_warp",
            device,
            error,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--backend", choices=("local", "microduck_rl", "both"), default="both")
    parser.add_argument(
        "--devices",
        default="cpu",
        help="Comma-separated devices, e.g. cpu,mps,cuda:0; microduck_rl MuJoCo-Warp omits mps",
    )
    ...
    parser.add_argument("--microduck-rl-root", type=Path, default=Path("../microduck_rl"))
    parser.add_argument(
    ...
    backends = ("local", "microduck_rl") if args.backend == "both" else (args.backend,)
    backend_devices = {
        backend: (
            [device for device in devices if _device_type(device) != "mps"]
            if backend == "microduck_rl"
            else devices
        )
        for backend in backends
    }
    if args.backend == "microduck_rl" and not backend_devices["microduck_rl"]:
        parser.error("microduck_rl MuJoCo-Warp has no MPS backend; use cpu or cuda instead")
    ...
    worker_index = 0
    for backend in backends:
        for device in backend_devices[backend]:
            worker_output = (
